#! /usr/bin/env xonsh
# ar18 Script version 2021-08-08_10:37:00
...
if not "AR18_PARENT_PROCESS" in ${...}:
  ...
  def log_header():
    ...
    return f"[{date_time()}] {os.path.realpath(c_stack.filename)}: {c_stack.function}():{c_stack.lineno}"
...
##################################SCRIPT_END###################################
    ...
    for message in $AR18_EXIT_MESSAGES:
      print(message)
